...
async def msg_advertence(message: discord.Message, logging: object, colors: ColorDiscord):
    '''
    Guarda registo y envia mensaje de advertencia al autor del mensaje.
    '''
    logging.warning(f'El usuario: {message.author.display_name}, ID: {message.author.id} se le dio advertencia de spam, mensaje: {message.content}')
